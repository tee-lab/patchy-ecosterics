...
if __name__ == '__main__':
    ...
    model_params.append([0.616, 0.618, 0.62])
    ...
    model_params.append([0.498, 0.5, 0.502])
    ...
    # model_datasets.append("100x100_residue")
    # model_params.append([500, 700, 830])
    ...
    for i in tqdm(range(len(models))):
        ...
        for j in range(len(model_param)):
            ...
            if row == 0 and j == 0:
                plt.semilogy(cluster_sizes, inverse_cdf, "b-", label=f"model")
                plt.semilogy(null_cluster_sizes, null_inverse_cdf, "0.7", label=f"null")
            else:
                plt.semilogy(cluster_sizes, inverse_cdf, "b-")
                plt.semilogy(null_cluster_sizes, null_inverse_cdf, "0.7")
    ...
    plt.savefig("fig2_exp.png", dpi=300)
    plt.show()