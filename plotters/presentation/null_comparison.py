...
if __name__ == '__main__':
    ...
    q = 0
    density = 0.61
    tdp_cluster_limit = 6000
    null_cluster_limit = 6000
    ...
    plt.legend()
    plt.savefig(f"p{p}_q{q}_cd.png")
    plt.show()

    tdp_file_name = tdp_folder + "_cluster_ds.txt"
    tdp_cluster_sizes, tdp_cluster_ds = get_mean_ds(tdp_path, tdp_folder, tdp_cluster_limit)
    null_file_name = null_folder + "_cluster_ds.txt"
    null_cluster_sizes, null_cluster_ds = get_mean_ds(null_path, null_folder, null_cluster_limit)
    ...
    plt.plot([0, max(tdp_cluster_limit, null_cluster_limit)], [0, 0], 'k--')
    plt.legend()
    plt.savefig(f"p{p}_q{q}_f.png")
    plt.show()

    tdp_file_name = tdp_folder + "_cluster_ds.txt"
    tdp_cluster_sizes, tdp_cluster_ds_sq = get_mean_ds_sq(tdp_path, tdp_folder, tdp_cluster_limit)
    null_file_name = null_folder + "_cluster_ds.txt"
    null_cluster_sizes, null_cluster_ds_sq = get_mean_ds_sq(null_path, null_folder, null_cluster_limit)

    plt.title("Diffusion Term", fontsize=title_size)
    plt.xlabel("s", fontsize=label_size)
    plt.ylabel("g^2 (s)", fontsize=label_size)
    plt.plot(tdp_cluster_sizes, tdp_cluster_ds_sq, label="TDP")
    plt.plot(null_cluster_sizes, null_cluster_ds_sq, label="Null")
    plt.legend()
    plt.savefig(f"p{p}_q{q}_g.png")
    plt.show()